...
|---|---|---|---|
| WPF | 5/5 | 優秀 | `x:Name` が直接 AutomationId にマッピングされる |
| WinForms | 4/5 | 良好 | `AccessibleName` = AutomationId |
| UWP / WinUI 3 | 5/5 | 優秀 | Microsoft の完全なサポート |
| Qt 6.x | 5/5 | 優秀 | アクセシビリティがデフォルトで有効；クラス名が `Qt6*` に変化 |
| Qt 5.15+ | 4/5 | 良好 | Accessibility モジュールが改善 |
| Qt 5.7–5.14 | 3/5 | 普通 | `QT_ACCESSIBILITY=1` が必要；objectName は手動 |
| Win32 / MFC | 3/5 | 普通 | コントロール ID にアクセス可能；テキスト一致が一般的 |
